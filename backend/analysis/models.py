...
class ScanRequest(BaseModel):
    ...
    contract_name: Optional[str] = Field(
        None, 
        description="Name of the contract. Auto-detected if not provided"
    )
    file_path: str = Field(
        ..., 
...
class ScanResult(BaseModel):
    ...
    findings: List[Finding] = Field(
        ...
        description="List of all security findings"
    )
    ...
    overall_score: int = Field(
        ...
        ge=0, 
        le=100,
        description="Security score from 0-100, where 100 is safest"
    )
    summary: str = Field(
        ...,
